...
const LinkItem = ({ href, path, children }) => {
  ...
  return (
    <NextLink href={href}>
      <Link p={2} bg={activeLink ? 'glassTeal' : undefined} color={activeLink ? '#202023' : inactiveColor}>
        {children}
      </Link>
    </NextLink>
  );
  ...
  return (
    <Box position="fixed" as="nav" w="100%" bg={useColorModeValue('#fffff40', '#20202380')} style={{ backdropFilter: 'blur(10px)' }} zIndex={1} {...props}>
      <Container display="flex" p={2} maxW={'container.md'} wrap="wrap" align="center" justify="space-between">
        ...
        <Stack direction={{ base: 'column', md: 'row' }} display={{ base: 'none', md: 'flex' }} width={{ base: 'full', md: 'auto' }} flexGrow={1} mt={{ base: 4, nmd: 0 }}>
